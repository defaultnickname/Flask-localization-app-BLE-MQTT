...
function dra(anchorsData, targetsData) {
    ...
    if (typeof targetsData !== 'undefined') {
        ...
        for (let i = 0; i < targetsData.length; i++) {
            createTarget(targetsData[i].x, targetsData[i].y)
        }
    }
...
// listen for mouse events
        ...
        function createTarget(x, y) {
            targets.push({x: x, y: y, width: 15, height: 15, fill: '#0000ff', isVisible: true});
            draw();
        }
        ...
        function deleteTarget(deleteId) {
            targets.splice(deleteId, 1);
            draw()
        }

        dra.deleteTarget = deleteTarget;

        function allTargetInvisible() {
            for (const e in targets) {
                e['isVisible'] = false;
                draw();
            }
        }

        dra.allTargetInvisible = allTargetInvisible;

        function allTargetVisible() {
            for (const e in targets) {
                e['isVisible'] = true;
            }
            draw();
        ...
        dra.allTargetVisible = allTargetVisible;
...
// handle mousedown events
        function myDown(e) {
            console.log('Anchors ', getAllRectangles())
            console.log('Targets ', getAllTargets())
